...
const Notifications = ({user, refresh, notifications}) => {
    ...
    const { delNotification} = useNotification();
        ...
        <div className="chats-list">
            {notif && notif.length > 0 ? (
                notif.map((notification, idx) => (
                    <>
                    <div key={notification.id}className={`chat ${!notification.is_read ? "unread" : ""}`} onClick={ () => readNotification(notification.id, delNotification, user?.token)}>
